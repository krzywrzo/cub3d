/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   hooks.c                                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: szmadeja <szmadeja@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/06/21 16:00:00 by szmadeja          #+#    #+#             */
/*   Updated: 2026/06/21 16:47:06 by szmadeja         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/cub3d.h"

#define MOVE_SPEED 0.05
#define ROT_SPEED 0.04

static int	is_wall(t_map_info *map, double x, double y)
{
    int	map_x;
    int	map_y;

    map_x = (int)x;
    map_y = (int)y;
    if (map_x < 0 || map_y < 0 || map_y >= map->map_height)
        return (1);
    if (map_x >= (int)ft_strlen(map->map_copy[map_y]))
        return (1);
    if (map->map_copy[map_y][map_x] == '1')
        return (1);
    return (0);
}

static void	check_and_move_x(t_game *game, double move_x)
{
    if (!is_wall(game->map, game->player->pos_x + move_x,
            game->player->pos_y))
        game->player->pos_x += move_x;
}

static void	check_and_move_y(t_game *game, double move_y)
{
    if (!is_wall(game->map, game->player->pos_x,
            game->player->pos_y + move_y))
        game->player->pos_y += move_y;
}

static void	rotate_player(t_player *p, double rot)
{
    double	old_dir_x;
    double	old_plane_x;

    old_dir_x = p->view_dir_x;
    p->view_dir_x = p->view_dir_x * cos(rot) - p->view_dir_y * sin(rot);
    p->view_dir_y = old_dir_x * sin(rot) + p->view_dir_y * cos(rot);
    old_plane_x = p->plane_x;
    p->plane_x = p->plane_x * cos(rot) - p->plane_y * sin(rot);
    p->plane_y = old_plane_x * sin(rot) + p->plane_y * cos(rot);
}

static void	handle_forward_backward(t_game *game)
{
    if (game->keys.w)
    {
        check_and_move_x(game, game->player->view_dir_x * MOVE_SPEED);
        check_and_move_y(game, game->player->view_dir_y * MOVE_SPEED);
    }
    if (game->keys.s)
    {
        check_and_move_x(game, -game->player->view_dir_x * MOVE_SPEED);
        check_and_move_y(game, -game->player->view_dir_y * MOVE_SPEED);
    }
}

static void	handle_strafe(t_game *game)
{
    if (game->keys.a)
        rotate_player(game->player, -ROT_SPEED);
    if (game->keys.d)
        rotate_player(game->player, ROT_SPEED);
}


int	key_press(int keycode, void *param)
{
    t_game	*game;

    game = (t_game *)param;
    if (keycode == KEY_ESC)
        game->keys.esc = 1;
    else if (keycode == KEY_W)
        game->keys.w = 1;
    else if (keycode == KEY_A)
        game->keys.a = 1;
    else if (keycode == KEY_S)
        game->keys.s = 1;
    else if (keycode == KEY_D)
        game->keys.d = 1;
    return (0);
}
int	key_release(int keycode, void *param)
{
    t_game	*game;

    game = (t_game *)param;
    if (keycode == KEY_ESC)
        game->keys.esc = 0;
    else if (keycode == KEY_W)
        game->keys.w = 0;
    else if (keycode == KEY_A)
        game->keys.a = 0;
    else if (keycode == KEY_S)
        game->keys.s = 0;
    else if (keycode == KEY_D)
        game->keys.d = 0;
    return (0);
}

int	game_loop(void *param)
{
    t_game	*game;

    game = (t_game *)param;
    if (game->keys.esc)
        close_app(game);
    handle_forward_backward(game);
    handle_strafe(game);
    render_scene(game);
    mlx_put_image_to_window(game->mlx, game->win, game->frame.img_ptr, 0, 0);
    return (0);
}
